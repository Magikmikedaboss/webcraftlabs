...
function NavLink({ href, label }: { href: string; label: string }) {
  return (
    <Link
      href={href}
      className="rounded-lg px-3 py-1.5 text-xs font-semibold text-[var(--text)] transition-all duration-200 hover:text-[var(--primary)] hover:bg-[var(--hoverSurface)]"
    >
      {label}
    </Link>
  );
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg hover:bg-[var(--hoverSurface)] hover:text-[var(--primary)] text-[var(--text)] bg-[var(--surface)] border border-[var(--border)]"
      >
        {label}
        <svg className="w-3 h-3 ml-1" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
      </button>
      {open && (
        <div id={dropdownId} role="menu" className="absolute left-0 mt-2 w-36 rounded-lg bg-[var(--surface)] shadow-lg border border-[var(--border)] z-[100]">
          {items.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="block px-4 py-2 text-xs text-[var(--text)] hover:bg-[var(--hoverSurface)] hover:text-[var(--primary)] rounded-lg"
    ...
    <div className={`min-h-screen ${bgClass} text-[var(--text)]`}>
      <header className={styles.header}>
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
          ...
          <Link href="/" className="flex items-center gap-2 font-bold text-sm sm:text-base tracking-tight text-[var(--text)] transition-opacity hover:opacity-90">
            <Image
        ...
        <div className="mx-auto max-w-7xl px-6 py-12">
          <div className="rounded-2xl bg-[var(--surface)] border border-[var(--border)] shadow-lg p-4 sm:p-6 md:p-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4 sm:gap-6 md:gap-8">
            <div className="flex items-center gap-4 mb-6 md:mb-0">
              ...
              <div>
                <div className="font-bold text-base sm:text-lg text-[var(--text)]">{SITE.name}</div>
