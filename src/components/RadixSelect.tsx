  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      <Select.Root value={value} onValueChange={onValueChange}>
        <Select.Trigger
          className={`inline-flex items-center justify-between w-full rounded-lg border px-3 sm:px-4 py-2 bg-white text-sm sm:text-base text-blue-900 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${
          ...
          <Select.Viewport className="p-1">
            {options.map((opt) => (
              <Select.Item
                key={opt.value}
                value={opt.value}
                className="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg cursor-pointer text-sm sm:text-base text-blue-900 hover:bg-blue-50 focus:bg-blue-100 select-none outline-none transition-all"
              >
